...
export function EcommerceTemplate({ siteData, subdomain }: EcommerceTemplateProps) {
  ...
  const handleValidateCart = () => {
    ...
    const productList = cartItems.map(item => 
      `- ${item.title} (Quantité: ${item.quantity}) - Prix: ${item.price || 'Non spécifié'} ${item.currency || siteData.defaultCurrency || ''}`
    ).join('\n');

    const totalOrderDetails = `Commande Totale :\n${productList}\n\nMontant Total: ${totalCartPrice} ${siteData.defaultCurrency || 'XOF'}`;
  ...
  const getLucideIcon = (iconName: string) => {
    const icons: { [key: string]: React.ElementType } = {
      ...
      StarHalf, CheckCircle, Plus, Minus, Trash2, Facebook, Instagram, MapPin
    };
  ...
  const CartModal = () => (
    ...
    >
      ...
      <div className={cn(
        "fixed top-0 right-0 w-full sm:w-[400px] h-full bg-card shadow-2xl transition-transform duration-300 p-6 flex flex-col",
              ...
              <div key={item.index} className="flex items-center gap-4 border-b pb-4 last:border-b-0" id={`cart-item-${item.index}`}>
                <div className="w-16 h-16 flex-shrink-0">
                  {item.image ? (
                    ...
                    <div className="w-full h-full bg-muted rounded-md flex items-center justify-center text-muted-foreground text-xs">No Image</div>
                  )}
                </div>
                <div className="flex-1">
                  <h3 className="font-semibold text-foreground">{item.title}</h3>
                  <p className={secondaryColorTextClass}>
                    {item.price} {item.currency || siteData.defaultCurrency || 'XOF'} x {item.quantity}
                  </p>
                  <p className="text-sm font-bold text-foreground">Total: {((item.price || 0) * item.quantity).toFixed(2)} {siteData.defaultCurrency || 'XOF'}</p>
                </div>
                ...
                <div className="flex items-center border border-border rounded-lg">
                  <button 
                    ...
                    className="p-1.5 text-foreground hover:bg-muted rounded-l-lg transition-colors"
                  ...
                  <button 
                    onClick={() => handleUpdateCartQuantity(item.index, item.quantity + 1)} 
                    className="p-1.5 text-foreground hover:bg-muted rounded-r-lg transition-colors"
        ...
        <div className="pt-4 border-t border-border">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className={cn("text-2xl font-extrabold", primaryColorTextClass)}>
              {totalCartPrice.toFixed(2)} {siteData.defaultCurrency || 'XOF'}
            </span>
          </div>
          <button 
            onClick={handleValidateCart} 
            disabled={cartItems.length === 0}
            className={cn("w-full px-5 py-3 rounded-lg font-bold text-primary-foreground text-lg transition-colors duration-300", primaryColorClass, primaryColorHoverBgClass, cartItems.length === 0 ? 'opacity-50 cursor-not-allowed' : '')}
  ...
  // --- Rendu du Composant ---
  return (
    <div className="font-sans antialiased text-foreground bg-background overflow-x-hidden" id="ecommerce-template-root">
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-7xl">
          <nav className="flex justify-between items-center py-4">
            ...
            <div className="hidden md:flex items-center gap-6" id="desktop-nav-menu">
              {sectionsVisibility.showProductsServices && products.length > 0 && <a href="#products" onClick={(e) => handleSmoothScroll(e, '#products')} className="text-foreground font-medium hover:text-primary transition-colors">Produits</a>}
              {sectionsVisibility.showProductsServices && services.length > 0 && <a href="#services" onClick={(e) => handleSmoothScroll(e, '#services')} className="text-foreground font-medium hover:text-primary transition-colors">Services</a>}
              ...
              {sectionsVisibility.showTestimonials && <a href="#testimonials" onClick={(e) => handleSmoothScroll(e, '#testimonials')} className="text-foreground font-medium hover:text-primary transition-colors">Avis</a>}
            ...
            <div className="flex items-center gap-4">
              ...
              >
                ...
                {totalCartCount > 0 && (
                  <span className={cn("absolute top-0 right-0 h-5 w-5 rounded-full text-xs font-bold text-primary-foreground flex items-center justify-center", secondaryColorClass)} id="cart-count-badge">
                    {totalCartCount}
                  </span>
                )}
              </button>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-card shadow-xl py-4 z-40" id="mobile-menu-overlay">
            <nav className="flex flex-col items-center gap-4">
              {sectionsVisibility.showProductsServices && products.length > 0 && <a href="#products" onClick={(e) => handleSmoothScroll(e, '#products')} className="text-foreground font-medium hover:text-primary transition-colors w-full text-center py-2 text-base">Produits</a>}
              {sectionsVisibility.showProductsServices && services.length > 0 && <a href="#services" onClick={(e) => handleSmoothScroll(e, '#services')} className="text-foreground font-medium hover:text-primary transition-colors w-full text-center py-2 text-base">Services</a>}
              ...
              {sectionsVisibility.showTestimonials && <a href="#testimonials" onClick={(e) => handleSmoothScroll(e, '#testimonials')} className="text-foreground font-medium hover:text-primary transition-colors w-full text-center py-2 text-base">Avis Clients</a>}
      ...
      {sectionsVisibility.showProductsServices && products.length > 0 && (
        <section id="products" className="py-12 bg-muted px-4">
          <div className="container mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" id="products-list">
              {products.map((product, index) => {
                ...
                return (
                  <div key={index} id={`product-card-${index}`} className="bg-card rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-1 group">
                    ...
                    <div className="p-4 flex flex-col justify-between h-[calc(100%-12rem)]">
                      <div>
                        <h3 className="text-xl font-bold mb-1 text-foreground" id={`product-title-${index}`}>{product.title}</h3>
                        <p className="text-muted-foreground text-sm mb-3 line-clamp-3" id={`product-description-${index}`}>{product.description}</p>
                      </div>

                      {product.price !== undefined && (
                        <p className={cn("text-2xl font-extrabold my-2", secondaryColorTextClass)} id={`product-price-${index}`}>
                          {product.price.toFixed(2)} {product.currency || siteData.defaultCurrency || 'XOF'}
                        </p>
                      )}

                      {/* Sélecteur de Quantité Modifiable */}
                      <div className="flex items-center justify-between gap-3 mt-3">
                        <label htmlFor={`quantity-${index}`} className="block text-foreground font-medium text-sm">Quantité:</label>
                        <input
                          type="number"
                          id={`quantity-${index}`}
                          name={`quantity-${index}`}
                          ...
                          className="w-16 px-2 py-1 border border-border rounded-lg text-center text-sm bg-background text-foreground"
                        />
                      </div>
                      ...
                      <button
                        ...
                        className={cn("mt-4 w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-semibold text-primary-foreground transition-colors duration-300", primaryColorClass, primaryColorHoverBgClass)}
                        id="btn-add-to-cart"
                      >
              ...
              {services.map((service, index) => {
                ...
                return (
                  <div key={index} id={`service-card-${index}`} className="bg-muted rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-1">
                    ...
                    <div className="p-4">
                      <h3 className="text-xl font-semibold mb-2 text-foreground" id={`service-title-${index}`}>{service.title}</h3>
                      <p className="text-muted-foreground text-sm mb-4 line-clamp-3" id={`service-description-${index}`}>{service.description}</p>
                      {service.price !== undefined && (
                        <p className={cn("text-lg font-bold mb-3", primaryColorTextClass)} id={`service-price-${index}`}>
                          À partir de {service.price} {service.currency || siteData.defaultCurrency || 'XOF'}
                        </p>
                      )}
          ...
          <div className="container mx-auto max-w-7xl">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="md:w-1/2" id="about-text-content">
                <p className="text-foreground text-base mb-4" id="about-paragraph-1">{siteData.aboutStory || "Notre mission est de vous offrir une sélection rigoureuse de produits de qualité. Nous nous engagons à un service client irréprochable et une expérience d'achat en ligne fluide et agréable."}</p>
                <p className="text-foreground text-base mb-4" id="about-paragraph-2">{(siteData.aboutStory && siteData.aboutStory.length > 150) ? siteData.aboutStory.substring(150) : "Chaque achat est une promesse de satisfaction et de durabilité. Découvrez la différence avec notre e-commerce."}</p>
                ...
                {skillsToDisplay.length > 0 && (
                    <Link 
                        ...
                        className={cn("inline-flex items-center gap-2 px-5 py-2 rounded-lg font-bold text-base text-secondary-foreground transition-all duration-300", secondaryColorClass, secondaryColorHoverBgClass, 'mt-4')}
                        id="btn-view-skills"
                    >
      ...
      {sectionsVisibility.showSkills && skillsToDisplay.length > 0 && (
        <section id="skills" className="py-12 bg-card px-4">
          <div className="container mx-auto max-w-7xl">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4" id="skills-list">
              {skillsToDisplay.map((skill, index) => {
                ...
                return (
                  <div key={index} id={`skill-item-${index}`} className="bg-muted rounded-lg shadow-md p-6 text-center space-y-3 border-t-4" style={{ borderColor: `var(--${siteData.primaryColor}-600)` }}>
                    <div className="flex items-center justify-center mb-4"><IconComponent className={cn("h-8 w-8", primaryColorTextClass)} /></div>
                    <h3 className="text-lg font-semibold text-foreground" id={`skill-title-${index}`}>{skill.title}</h3>
                    <p className="text-muted-foreground text-sm" id={`skill-description-${index}`}>{skill.description}</p>
                  </div>
                );
              })}
            </div>
      ...
      {sectionsVisibility.showTestimonials && testimonialsToDisplay.length > 0 && (
        <section id="testimonials" className="py-12 bg-muted px-4">
          <div className="container mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6" id="testimonials-list">
              {testimonialsToDisplay.map((testimonial, index) => (
                <div key={index} id={`testimonial-card-${index}`} className="bg-card rounded-xl p-6 shadow-lg relative border-l-4" style={{ borderColor: `var(--${siteData.secondaryColor}-500)` }}>
                  <Star className={cn("h-5 w-5 fill-yellow-400 text-yellow-400 mb-2", secondaryColorTextClass)} />
                  <p className="italic text-foreground mb-4 text-base" id={`testimonial-quote-${index}`}>"{testimonial.quote}"</p>
                  <div className="flex items-center gap-3">
                    {testimonial.avatar && (
                        <Image src={testimonial.avatar} alt={testimonial.author} width={40} height={40} className="rounded-full object-cover" />
                    )}
                    <div>
                      <p className="font-bold text-foreground" id={`testimonial-author-${index}`}>{testimonial.author}</p>
                      <p className="text-sm text-muted-foreground" id={`testimonial-location-${index}`}>{testimonial.location}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {sectionsVisibility.showContact && siteData.showContactForm && (
        <section id="contact" className="py-12 bg-card px-4">
          <div className="container mx-auto max-w-7xl">
              ...
              <div id="contact-form-block">
                ...
                <form onSubmit={handleSubmit} className="space-y-4" id="contact-form">
                  ...
                  <div className="form-group" id="form-group-service-display">
                    <label htmlFor="service-display" className="block text-foreground font-medium mb-1 text-sm">Objet de la demande</label>
                    <input 
                        type="text" 
                        id="service-display" 
                  ...
                  <div className="form-group" id="form-group-message">
                    <label htmlFor="message" className="block text-foreground font-medium mb-1 text-sm">Message / Détail de la commande</label>
